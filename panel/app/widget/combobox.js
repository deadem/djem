...
Ext.define('djem.widget.combobox', {
  ...
  afterRender: function() {
    ...
    me.getEl().on('click', function() {
      if (me.queryMode !== 'remote' || me.getStore().isLoaded()) {
        ...
        me.getStore().load({ callback: function() { me.expand(); } });
      }
    });
  ...
  initStore: function() {
    ...
    if (me.queryMode == 'remote') {
      ...
      store.getProxy().setExtraParams({ _doctype: view.config.data._doctype, id: view.config.data.id, field: me.name });
    }
  },

  listeners: {
    added: function() { return this.initStore(); },
    initStore: function() { return this.initStore(); },
