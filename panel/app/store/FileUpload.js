...
Ext.define('djem.store.FileUpload', {
  ...
  lock: function(file) {
    ...
    var record = { url: url, file: file, name: file.name };
    record.get = function(name) { return record[name]; };
    return record;
  },
  ...
  upload: function(records, callback) {
    ...
    xhr.onreadystatechange = function() {
      ...
      if (xhr.readyState == 4) {
        ...
        if (xhr.status == 500) {
          ...
          djem.app.fireEvent('initSession', { success: function() { me.upload(records, callback); } });
          return;
        }
        callback(false);
      }
    ...
    Ext.each(records, function(record) { form.append('data[]', record.get('file')); });
    xhr.send(form);
  },
  ...
  destroy: function() {
    ...
    Ext.each(me.lockedUrls, function(obj) { window.URL.revokeObjectURL(obj); });
  }
});
