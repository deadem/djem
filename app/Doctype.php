...
/*
example route:
    Route::get('{any}', function($url) { return App\Doctypes\Doctype::view($url, App\Models\Url::class); })->where('any', '(?!djem/).*');
...
class Doctype extends \Illuminate\Routing\Controller
{
     ...
     */
    public static function view($url, $urlModel)
    {
        return self::viewAddress($urlModel::where('url', '=', $url)->first());
    }
     ...
     */
    public static function viewAddress($address)
    {
        if (!$address) {
            abort(404, 'File not found');
        }
        $doctype = self::getDocType($address->doctype);
        return $doctype::renderView($address->model, $address->refid);
    }
     ...
     * @return object данные для отображения 
     */
    static public function renderView($model, $id)
    {
